...
impl <'a> SqlTokenizer<'a> {
    pub fn new(sql: &str) -> SqlTokenizer {
        let token_parsers: Vec<Box<TokenParser>> = vec![
            ...
            Box::new(KeywordTokenParser{ text: "full", sql_type: SqlType::Full }),
            Box::new(KeywordTokenParser{ text: "left", sql_type: SqlType::Left }),
            Box::new(KeywordTokenParser{ text: "right", sql_type: SqlType::Right }),
